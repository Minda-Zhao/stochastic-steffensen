...
# Choose 0 and 8 for binary classification
index = [i for i in range(len(y_train)) if y_train[i] == 0 | y_train[i] == 8]
...
for i in range(repeat):
    ...
    for j in range(len(w_path_ssbb)):
        ...
        error_ssbb[j, i] = error
error_ssbb = np.mean(error_ssbb, axis=1)
plt.semilogy(error_ssbb, linewidth=2, label='b = 32', marker='v', color='r')
...
plt.xlim((0, 30))
plt.ylim((1e-17, 1))
plt.savefig('./plot/logistic/batch.png')
plt.show()
...
for i in range(repeat):
    w_path_sgd, time_stamp_sgd = alg.sgd(func_sto, initial_w, epoch_sgd, m, 0.5, b, n)
...
for i in range(repeat):
    w_path_svrg, time_stamp_svrg = alg.svrg(func_sto, initial_w, epoch_svrg, m, 0.1, b, n)
...
plt.xlim((0, 60))
plt.ylim((1e-17, 1))
plt.savefig('./plot/logistic/passes.png')
plt.show()
...
plt.xlim((0, 1))
plt.ylim((1e-17, 1))
plt.savefig('./plot/logistic/time.png')
plt.show()